...
- File saving: `.yml` files are stored in the services directory. Deletion of files is strictly forbidden. If a service file is deleted, the script will restore it from a previous version of the `services.json` file. It is also forbidden to change service `id` inside the `.yml` file.
